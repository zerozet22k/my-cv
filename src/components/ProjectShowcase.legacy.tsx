      ...
      <div className="p-4 flex flex-col justify-between flex-grow">
        <div>
          <h3 className="text-lg font-bold text-gray-800 flex items-center mb-1">
            {title}
            <span className="ml-2 text-sm">{renderPlatformIcons(platform)}</span>
          </h3>
...
const ProjectWindow: React.FC<ProjectWindowProps> = ({
  ...
  return (
    ...
    >
      ...
      >
        <span className="text-sm font-semibold truncate window-title">{title}</span>
      ...
      {!isMinimized && (
        <div className="relative flex-grow">
          <iframe src={projectUrl} title={title} className="w-full h-full border-none" />
          {!isMaximized && (
            <div
...
export const ProjectShowcaseBackup: React.FC<ProjectShowcaseProps> = ({ projects, color = "#3B82F6" }) => {
...
export default ProjectShowcaseBackup;
